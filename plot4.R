...
with(subset(PowerData_NoNA, Date == "2007-02-01" | Date == "2007-02-02"), plot(
  DateTime, Global_active_power, type = "l", xlab = " ", ylab = "Global Active Power", xaxt = "n"))
axis(side = 1, at = breaks, labels = c("Thu", "Fri", "Sat"))


with(subset(PowerData_NoNA, Date == "2007-02-01" | Date == "2007-02-02"), 
     plot(DateTime, Sub_metering_1, type = "l", xlab = " ", ylab = "Energy sub metering", xaxt = "n"))
with(subset(PowerData_NoNA, Date == "2007-02-01" | Date == "2007-02-02"), 
     lines(DateTime, Sub_metering_2, col = "red"))
with(subset(PowerData_NoNA, Date == "2007-02-01" | Date == "2007-02-02"), 
     lines(DateTime, Sub_metering_3, col = "blue"))
axis(side = 1, at = breaks, labels = c("Thu", "Fri", "Sat"))
legend("topright", bty = "n", cex = 0.8, lty = c(1, 1, 1), col = c("black", "red", "blue"), legend = c("Sub_metering_1", "Sub_metering_2", "Sub_metering_3"))

with(subset(PowerData_NoNA, Date == "2007-02-01" | Date == "2007-02-02"), plot(
  DateTime, Voltage, type = "l", xaxt = "n"))
axis(side = 1, at = breaks, labels = c("Thu", "Fri", "Sat"))

with(subset(PowerData_NoNA, Date == "2007-02-01" | Date == "2007-02-02"), plot(
  DateTime, Global_reactive_power, type = "l", xlab = " ", xaxt = "n"))
